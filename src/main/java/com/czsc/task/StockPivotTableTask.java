...
@Slf4j
@Component
public class StockPivotTableTask {
    ...
    public List<StockPivotDto> dealData(List<StockSegDto> stockSegDtos, List<StockBiDto> stockBiDtos, List<StockContainsTable> stockContainsTables) {
//        List<StockSegDto> stockSegDtos = stockSegTableService.getStockSegDtosBySymbol("000001.XSHG");
        ...
        List<StockPivotDto> stockPivotDtos = getPivot(stockSegDtos, stockContainsTables);
        processPivot(stockPivotDtos);
        return stockPivotDtos;
    }

    private void processPivot(List<StockPivotDto> stockPivotDtos) {
        for (int i = 0; i < stockPivotDtos.size() - 1; i++) {
            StockPivotDto stockPivotDto = stockPivotDtos.get(i);
            StockPivotDto stockPivotDto1 = stockPivotDtos.get(i + 1);
    ...
    private List<StockPivotDto> getPivot(List<StockSegDto> stockSegDtos, List<StockContainsTable> stockContainsTables) {
        List<StockPivotDto> stockPivotDtos = new ArrayList<>();
        int i = 0;
            ...
            if (i < stockSegDtos.size() - 3) {
                ...
                if (type == FenxingType.TOP_PART.getValue()) {
                    if (stockSegDto3.getEndPrice().compareTo(stockSegDto1.getStartPrice()) <= 0) {
                        StockPivotDto stockPivotDto = buildNormalStockPivotDto(new ArrayList<>(stockSegDtos.subList(i, i + 4)), stockContainsTables, type);
                        int i_j = 1;
                        ...
                        StockPivotDto stockPivotDto = buildSimilarStockPivotDto(new ArrayList<>(stockSegDtos.subList(i, i + 3)), stockContainsTables, type);
                        stockPivotDto.setType(stockPivotDto.getEnterSegDto().getType() + "_" + stockPivotDto.getLeaveSegDto().getType());
                        i += stockPivotDto.getSize();
                        stockPivotDtos.add(stockPivotDto);
                        continue;
    ...
    private StockPivotDto buildNormalStockPivotDto(List<StockSegDto> stockSegDtos, List<StockContainsTable> stockContainsTables, Integer type) {
        StockPivotDto stockPivotDto = new StockPivotDto();
    ...
    private StockPivotDto buildSimilarStockPivotDto(List<StockSegDto> stockSegDtos, List<StockContainsTable> stockContainsTables, Integer type) {
        StockPivotDto stockPivotDto = new StockPivotDto();
    ...
    private void pivotGrow(StockPivotDto stockPivotDto, StockSegDto stockSegDto, List<StockContainsTable> stockContainsTables) {
